...
#define AACPSY_LONG         1024
#define AACPSY_SHORT        128
#define AACPSY_TRANS_OFFSET 448  // (1024-128)/2 = 448
...
typedef struct _fa_aacpsy_t {
    ...
    float mag_prev1_long[AACPSY_LONG];
    float mag_prev2_long[AACPSY_LONG];
    float phi_prev1_long[AACPSY_LONG];
    float phi_prev2_long[AACPSY_LONG];

    float mag_prev1_short[AACPSY_SHORT];
    float mag_prev2_short[AACPSY_SHORT];
    float phi_prev1_short[AACPSY_SHORT];
    float phi_prev2_short[AACPSY_SHORT];
...
uintptr_t fa_aacpsy_init(int sample_rate)
{
    ...
    /*f->bit_alloc   = 0;*/
    /*f->block_type  = AACPSY_LONG_BLOCK;*/
...
float fa_aacpsy_calculate_pe(uintptr_t handle, short *x, int block_type)
{
    ...
    if(block_type == AACPSY_LONG_BLOCK) {
        fa_psychomodel2_calculate(f->h_psy2_long , x, &pe);
        pe_sum = pe;
    }else {
        pe_sum = 0;
        for(win = 0; win < 8; win++) {
            xp = x + AACPSY_TRANS_OFFSET + win*128;
            fa_psychomodel2_calculate(f->h_psy2_short, xp, &pe);
            pe_sum += pe;
        }
    }
...
/*
int fa_aacpsy_get_block_type(uintptr_t handle)
{
    fa_aacpsy_t *f = (fa_aacpsy_t *)handle;
...
void fa_aacpsy_calculate(uintptr_t handle, short *x)
{
    int   win;
    short *xp;
    float pe;
    ...
    if(f->block_type == AACPSY_LONG_BLOCK) {
        fa_psychomodel2_calculate(f->h_psy2_long , x, &pe);
        pe_sum = pe;
        f->bit_alloc = 0.3 * pe_sum + 6.0 * sqrt(pe_sum);
    }else {
        pe_sum = 0;
        for(win = 0; win < 8; win++) {
            xp = x + AACPSY_TRANS_OFFSET + win*128;
        ...
        f->bit_alloc = 0.6 * pe_sum + 24.0 * sqrt(pe_sum);
    }

    if(f->bit_alloc < 0)    f->bit_alloc = 0;
    if(f->bit_alloc > 3000) f->bit_alloc = 3000;
}
*/
